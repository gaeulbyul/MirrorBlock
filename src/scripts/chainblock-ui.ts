...
async function mobConfirm(message: string): Promise<boolean> {
  ...
  Object.assign<CSSStyleDeclaration, Partial<CSSStyleDeclaration>>(
    rootElem.style,
    {
      ...
      width: '40vw',
      height: '200px',
      boxShadow: '0 0 20px crimson', // XXX ?????
    }
  )
  const shadowRoot = rootElem.attachShadow({ mode: 'closed' })
  shadowRoot.innerHTML = `
    <div class="mobconfirm">
      <div class="title">체인맞블락 &ndash; Mirror Of Block</div>
  ...
  `
  shadowRoot.querySelector('.message')!.textContent = message
  document.body.appendChild(rootElem)
  return new Promise<boolean>(resolve => {
    shadowRoot.querySelector('button.ok')!.addEventListener(
      'click',
      event => {
    ...
    shadowRoot.querySelector('button.cancel')!.addEventListener(
      'click',
      event => {
        event.preventDefault()
        rootElem.remove()
